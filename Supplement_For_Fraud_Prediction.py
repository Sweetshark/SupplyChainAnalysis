...
# On the base of Fraud order and late delivery prediction, this chapter also explains feature importance in decisionTree model; uses voting to do model combination.
...
# ## Another information from Decision Tree：feature importance

importance_col = dt_fraud.feature_importances_.argsort()
# print(importance_col)
# print(dt_fraud.feature_importances_)
feat_importance = pd.DataFrame({'feature':fraud_x.columns[importance_col],'importance':dt_fraud.feature_importances_[importance_col]})
# feat_importance

# barplot
feat_importance.sort_values(by='importance',ascending=True)
plt.figure(figsize=(20,8))
sns.catplot(x='feature',y='importance',data = feat_importance,kind ='bar',height=6,aspect=2)
plt.title('Feature Importance in DecisionTree Model for Fraud Detection',fontsize=16)
...
eclf_fraud = VotingClassifier(estimators=[('LR',lr_fraud),('DT',dt_fraud),('XGB',xgb_fraud)],voting='hard')
for clf, label in zip([lr_fraud,dt_fraud,xgb_fraud],['LR','DT','XGB','Voting']):
    score = cross_val_score(clf, fraud_x,fraud_y,cv=5, scoring='roc_auc')
    print("AUC score: %0.2f(+/- %0.2f)[%s]"%(score.mean(),score.std(),label))


# SOFT VOTING
from sklearn.ensemble import VotingClassifier
from sklearn.model_selection import cross_val_score
eclf_fraud = VotingClassifier(estimators=[('LR',lr_fraud),('DT',dt_fraud),('XGB',xgb_fraud)],voting='soft')
for clf, label in zip([lr_fraud,dt_fraud,xgb_fraud],['LR','DT','XGB','Voting']):
    score = cross_val_score(clf, fraud_x,fraud_y,cv=5, scoring='f1')
    print("F1 score: %0.2f(+/- %0.2f)[%s]"%(score.mean(),score.std(),label))
